...
# Note: small customization w.r.t. builtin haddnano.py,
# to ignore objects that are not TTrees (giving errors on merging).
import os
import sys
import ROOT
import numpy
...
for e in fileHandles[0].GetListOfKeys():
    name = e.GetName()
    # customization: skip unrecognized names
    if name not in ['Events', 'LuminosityBlocks', 'Runs']:
        msg = f'Found object with unrecognized name "{name}";'
        msg += ' skipping merging this object...'
        print(msg)
        continue
    msg = f'Found object "{name}", now merging...'
    print(msg)
    obj = e.ReadObj()
